...
** Emacs per-project configuration
  :PROPERTIES:
  :header-args: :tangle (concat (org-entry-get nil "project-root" t) "emacs.el")
  :END:
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'load-path (expand-file-name "<<ROOT>>/.env/elisp/autoload"))
...
** ZSH per-project configuration
  :PROPERTIES:
  :header-args: :tangle (concat (org-entry-get nil "project-root" t) "zsh.zsh")
  :END:
